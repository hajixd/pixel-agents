...
function processCodexTranscriptLine(
	...
	if (record.type === 'event_msg') {
		...
		if (eventType === 'user_message' || eventType === 'turn_aborted') {
			if (eventType === 'user_message') {
				...
				if (text) {
					send({ type: 'agentMessage', id: agentId, role: 'user', text, timestamp: Date.now() });
				}
			}
			cancelWaitingTimer(agentId, waitingTimers);
	...
	if (payloadType === 'message' && payload.role === 'assistant') {
		const text = extractCodexMessageText(payload);
		if (text) {
			send({ type: 'agentMessage', id: agentId, role: 'assistant', text, timestamp: Date.now() });
		}
	...
	try {
		...
		if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
			...
			const assistantText = blocks
				...
				.trim();
			if (assistantText) {
				send({ type: 'agentMessage', id: agentId, role: 'assistant', text: assistantText, timestamp: Date.now() });
			}
			...
			if (Array.isArray(content)) {
				const userText = content
					...
					.trim();
				if (userText) {
					send({ type: 'agentMessage', id: agentId, role: 'user', text: userText, timestamp: Date.now() });
				}
				...
				send({ type: 'agentMessage', id: agentId, role: 'user', text: content.trim(), timestamp: Date.now() });
				// New user text prompt — new turn starting
				cancelWaitingTimer(agentId, waitingTimers);
				clearAgentActivity(agent, agentId, permissionTimers, send);
				agent.hadToolsInTurn = false;
