...
import { processTranscriptLine } from './transcriptBridge.js';
...
export function readNewLines(
	...
	try {
		...
		if (hasLines) {
			send({ type: 'agentHeartbeat', id: agentId, timestamp: Date.now(), jsonlFile: agent.jsonlFile });
...
export function resyncAgent(
	...
	const agent = agents.get(agentId);
	if (!agent) {
		send({ type: 'agentResynced', id: agentId, ok: false, reason: 'Agent not found' });
		return;
	}
	...
	startFileWatching(agentId, agent.jsonlFile, agents, fileWatchers, pollingTimers, waitingTimers, permissionTimers, send);
	readNewLines(agentId, agents, waitingTimers, permissionTimers, send);
	...
	send({ type: 'agentResynced', id: agentId, ok: true, jsonlFile: agent.jsonlFile });
}
